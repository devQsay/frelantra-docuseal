...
RUN set -e && \
    ...
    sed -i 's/ESIGN_CERTS_KEY)\.value/ESIGN_CERTS_KEY)\&.value/g' /app/lib/accounts.rb && \
    # accounts.rb: guard against nil cert_data
    sed -i '/if (default_cert/i\    return Docuseal.default_pkcs if cert_data.blank?' /app/lib/accounts.rb && \
    # docuseal.rb: guard against empty CERTS hash
    sed -i "/CERTS\['enabled'\] == false/a\\    return if Docuseal::CERTS.blank? || Docuseal::CERTS['cert'].blank?" /app/lib/docuseal.rb && \
    # storage.yml: remove explicit AWS credentials (use IAM role)
    sed -i '/access_key_id:/d; /secret_access_key:/d' /app/config/storage.yml
